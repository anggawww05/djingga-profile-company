...
@section('container')
    <div class="mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-[#23272F]">Tambah Konsultasi Baru</h1>
                <p class="text-gray-600 mt-1">Isi form di bawah untuk menambahkan data konsultasi</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-lg border border-[#52a08a]/10 overflow-hidden">
        <div class="p-8">
            <form action="{{ route('manage-consultation.store') }}" method="POST">
                @csrf
                ...
                <div class="mt-6">
                    ...
                    <select name="service" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:border-[#52a08a] transition-colors">
                        ...
                        @foreach(($services ?? []) as $s)
                            <option value="{{ optional($s)->id ?? $s }}" @selected(old('service') == (optional($s)->id ?? $s))>{{ optional($s)->service_name ?? $s }}</option>
                        @endforeach
                    </select>
                </div>
